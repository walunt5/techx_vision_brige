...
#  配置区域 —— 只需要改这里！
# ═══════════════════════════════════════════════════════════════
SEND_TO_IP   = "192.168.10.100"   # PC 的网口 IP ← 改这里！
...
def build_packet(seq: int, track_id: int) -> bytes:
    ...
    时间戳字段：float64 Unix 纪元秒数（time.time()）。
    GMK 接收端 timestamp_mode="auto" 会校验该值 >= 1500000000.0（~2017年），
    若 Jetson 使用其他时钟源（如 monotonic），接收端会自动回退为本地时间。
    """
    timestamp = time.time()
    # 模拟一个正弦运动的假目标
    t = timestamp * 0.5
